...
GUICtrlCreateLabel("MetaboLabPy path", 10, 70, 120, 20)
$metabolabpyPath = GUICtrlCreateEdit("", 140, 70, 270, 20)
$setMetabolabpyPath = GUICtrlCreateButton("Select", 420, 70, 70, 20)
...
while 1
   ...
   Select
	  ...
	  case $msg = $nextButton
		 ...
		 Else
			If GUICtrlRead($metabolabpyPath) = "" Then
			   warningMetabolabpyPathEmpty()
			Else
			   ExitLoop
			EndIf
		 EndIf
...
$metabolabpyDir = GUICtrlRead($metabolabpyPath)
...
WinSetOnTop($gui2, "", 1)
GUISetState() ; show GUI
InstallMetaboLabPy($installDir, $installMinicondaVar, $addToPath, $registerPython, $metabolabpyDir)
...
FUNC _select_folder2()
    $sDir = FileSelectFolder("Select MetaboLabPy git repository folder", "", 0)
    If @error <> 1 Then
        GUICtrlSetData($metabolabpyPath,$sDir)
    Else
        ToolTip("user abort",0,0)
        Sleep(500)
        ToolTip("",0,0)
...
Func InstallMetaboLabPy($installDir, $installMinicondaVar, $addToPath, $registerPython, $metabolabpyDir)
...
; Delete the file.
   ...
   EnvUpdate()
   $cmdStr = "cmd.exe" & ' /c "' & $condaExe & ' init cmd.exe"'
   RunWait($cmdStr)
   ...
   RunWait($cmdStr)
   $msgbData = GUICtrlRead($messageBoard) & "Installing Python 3.7.4..." & @CRLF
   GUICtrlSetData($messageBoard, $msgbData)
   $cmdStr = "cmd.exe" & ' /c "' & $condaExe & ' install --name metabolabpy python=3.7.4 -y"'
   RunWait($cmdStr)
   $msgbData = GUICtrlRead($messageBoard) & "Installing NumPy with Intel acceleration..." & @CRLF
   GUICtrlSetData($messageBoard, $msgbData)
   $cmdStr = "cmd.exe" & ' /c "' & $condaExe & ' install --name metabolabpy numpy -c intel --no-update-deps -y"'
   RunWait($cmdStr)
   $msgbData = GUICtrlRead($messageBoard) & "Installing PySide2, matplotlib and scipy..." & @CRLF
   GUICtrlSetData($messageBoard, $msgbData)
   $cmdStr = "cmd.exe" & ' /c "' & $condaExe & ' activate && pip install incremental pyside2 matplotlib scipy -y"'
   RunWait($cmdStr)
   $msgbData = GUICtrlRead($messageBoard) & "Installing MetaboLabPy..." & @CRLF
   GUICtrlSetData($messageBoard, $msgbData)
   RunWait("cmd.exe" & ' /c "conda activate metabolabpy&&python setup.py develop"', $metabolabpyDir, @SW_SHOW)
   $msgbData = GUICtrlRead($messageBoard) & "Creating Starter Script and Desktop lnk..." & @CRLF
   GUICtrlSetData($messageBoard, $msgbData)
   $batFileHandle = FileOpen($installDir & "\ml.bat", 2)
   FileWrite($batFileHandle, "start /min ml_exec.bat")
   ...
   $sFilePath = @DesktopDir & "\MetaboLabPy.lnk"
   FileCreateShortcut($installDir & "\ml.bat", $sFilePath, @DesktopDir, "", "", $metabolabpyDir & "\metabolabpy\icon\icon.ico", @SW_SHOWMINIMIZED)
